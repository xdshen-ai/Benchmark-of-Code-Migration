...
def Random_code_id2(code_id_path, input_path, num, item_output_path,package_output_path):
    # 获取所有 code_id 和 data 数据
    code_ids = getDictFromJsonl(code_id_path)  # 假设这个是一个包含 code_id 的列表
    data = getDictFromJsonl(input_path)
    ...
    for code_id in code_ids:
        if code_id["code_id"] in data_by_code_id:
            ...
            if compare_versions(items[0]['origin_version'],items[0]['compare_version']) == -1:
                select_items = random.sample(items, min(num, len(items))) 
            else:
                try:
                    items.sort(key = lambda x : x["re_run_answer"])
                    select_items = items[0:num]
                except:                
                    select_items = random.sample(items, min(num, len(items))) 

            if compare_versions(select_items[0]['origin_version'],select_items[0]['compare_version']) == -1:
                if code_id_pass(select_items,1):
                    old2new_pass_code_id_num = old2new_pass_code_id_num +1
                    data_by_package[select_items[0]['package']] = data_by_package[select_items[0]['package']] + 1
            else:
                if code_id_pass(select_items,1):
                    new2old_pass_code_id_num =new2old_pass_code_id_num +1
                    data_by_package[select_items[0]['package']] = data_by_package[select_items[0]['package']] + 1
...
def Random_code_id4(code_id_path, input_path, num, target_old2new_num,target_new2old_num,item_output_path,package_output_path):
    # 获取所有 code_id 和 data 数据
    code_ids = getDictFromJsonl(code_id_path)  # 假设这个是一个包含 code_id 的列表
    data = getDictFromJsonl(input_path)
    ...
    for code_id in code_ids:
        if code_id["code_id"] in data_by_code_id:
            ...
            # 随机选择 num 个不重复的记录
            if compare_versions(items[0]['origin_version'],items[0]['compare_version']) == 1:
                if new2old_pass_code_id_num < target_new2old_num :
                    items.sort(key=lambda x: x.get("re_run_answer", 0), reverse=True)
                    selected_items = items[0:num]
                else:
                    items.sort(key=lambda x: x.get("re_run_answer", 0))
                    selected_items = items[0:num]
            else:
                if old2new_pass_code_id_num < target_old2new_num :
                    items.sort(key=lambda x: x.get("re_run_answer", 0), reverse=True)
                    selected_items = items[0:num]
                else:
                    items.sort(key=lambda x: x.get("re_run_answer", 0))
